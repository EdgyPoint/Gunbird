...
update_status ModulePlayer2::Update()
{
	...
	if (App->input->keyboard[SDL_SCANCODE_O] == KEY_STATE::KEY_REPEAT && SDL_GetTicks() >= shot)
	{
		...
		App->particles->AddParticle(App->particles->marionbeam[beam++], position.x + 11, position.y - 25, COLLIDER_PLAYER2_SHOT);
		if (beam == 3) beam = 0;
		App->particles->AddParticle(App->particles->marionbeam[beam++], position.x + 11, position.y + 5, COLLIDER_PLAYER2_SHOT, 140);
		if (beam == 3) beam = 0;
		App->particles->AddParticle(App->particles->marionbeam[beam++], position.x + 11, position.y + 35, COLLIDER_PLAYER2_SHOT, 280);
		if (beam == 3) beam = 0;
		App->particles->AddParticle(App->particles->marionbeam[beam++], position.x + 11, position.y + 65, COLLIDER_PLAYER2_SHOT, 420);
		if (beam == 3) beam = 0;
